...
const cache = require('./dynamic-cache');
...
class DynamixObject extends BaseObject
{
    ...
    update (datas)
    {
        ...
        for (let field in datas)
        {
            if (!store.hasOwnProperty(field)) continue;
...
function normalizeFieldType (type, normalizedField)
{
    ...
    {
        case 'function':
            if (type.prototype instanceof DynamicObject)
            {
