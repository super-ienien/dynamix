...
var exists = function (type, arg1)
{
	...
	if (typeof id !== 'undefined' && _cache[type].hasOwnProperty(id))
	{
...
function getOneByOrCreate (type, search, data)
{
    ...
    if (typeof type == 'function') type = type.name;
    var instance = get(type, util.mixin (data, search));
    for (var i in search)
    {
...
var all = function (type, limit)
{
    ...
    return util.copy(_cache[type], limit);
};
...
var register = function (constructor, params)
{
	...
	var index = params.hasOwnProperty('index') ? (utilNode.isArray(params.index) ? params.index:[params.index]):[];
	var idProperty = params.hasOwnProperty('id') ? params.id:_defaultIdProperty;
