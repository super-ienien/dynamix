...
const enabled = new Symbol('enabled');
const pendingConnections = new Symbol('pendingConnections');
const namespaces = new Symbol('namespaces');
const io = new Symbol('io');
...
class Server
{
     ...
     * @param {HttpServer} options.httpServer
     * @param {boolean} [options.enabled=false] - wether connections are accepted or not. If false connections attempts will be queued until enabled is called
     * @param {Object} [options.socketio=false] - socketio options please refer to socketio documentation
     * @param {Remoted} [options.root] - Root remoted object. This object will be used as root for get requests on default the namespace
     */
    constructor(options)
    {
        if (!options.httpServer) throw new Error ('you must provide an httpServer handle');
        this[enabled] = !!options.enabled;
        this[io] = require('socket.io')(options.httpServer, options.socketio);
        io.use(function (socket, next)
        {
            if (this.enabled) return next();
            this[pendingConnections].push(next);
        });
    }
    ...
    {
        this[enabled] = true;
        this[pendingConnections].forEach(next => next());
        this.delayedConnections = [];
    }
     ...
     * @param {Remoted} [root] - Root remoted object. This object will be used as root for get requests on the namespace
     */
    addNamespace (namespace, root)
    {
        if (this[namespaces].has(name)) throw new Error ('Namespace "'+name+'" already exists');
        this[namespaces] = this[io].of(namespace);
    }
}
 ...
 *
 * @returns {*|Int16Array|Int8Array|Uint32Array|(*|string)[]|Int32Array}
 */
