...
function parseIdString(idStr) {
	...
	return {_type: splitted[0], _id: splitted[1], error: splitted.length !== 2};
}
...
exports.wordsPattern = function (words, minMark, maxMark)
{
    let wordsStr = words.map((word) => word.startsWith('/') && word.endsWith('/') ? word.slice(1,-1):_.escapeRegExp(word)).sort().join('|');
    if (typeof minMark === 'number' && typeof maxMark === 'number')
    {
        let reStr;
        let markStr = `(?:${toRegexRange(minMark, maxMark)})(?:(?:\\.|,)[0-9]+)?`;
        if (words.length)
        {
            reStr = `\\b(?:${wordsStr})\\D*?(${markStr})(?:\\D|$)`;
        }
        else
        {
            reStr = `(?:^|\\D)(${markStr})(?:\\D|$)`;
...
exports.wordsRegExp = function (words, minMark, maxMark)
{
    return new RegExp(this.wordsPattern(words, minMark, maxMark), 'i');
};

exports.markRegExp = function (minMark, maxMark)
{
    if (typeof minMark === 'number' && typeof maxMark === 'number')
    {
        return new RegExp(`(?:^|\\D)((?:${toRegexRange(minMark, maxMark)})(?:(?:\\.|,)[0-9]+)?)(?:\\D|$)`);
    }
	return null;
};