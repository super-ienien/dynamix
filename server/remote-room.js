...
function RemoteRoom (client)
{
    ...
    if (client)
    {
        console.log ('CREATE '+this.__static.name+' FOR CLIENT : '+client.name);
    }
    else
    {
    ...
    //public
    this.client = client;
    this.global = this.client ? false:true;
    this.id = this.global ? this.__static.name : this.__static.name+'-'+client._id;
    this.sockets = {};
...
RemoteRoom._removeFromCache = function (room)
{
    ...
    delete RemoteRoom.__cache[this.name][room.client.id];
};
...
RemoteRoom.getOne = function (p)
{
    ...
    if (typeof p === 'object' && p.hasOwnProperty('client') && p.client instanceof Client)
    {
        ...
        var room = new this(p.client);
        RemoteRoom.__cache[this.name][room.client.id] = room;
